...
function capToFront(s) {
  ...
  //Escribe tu código aquí
  let nuevoOrderMayusculas = '';
  let nuevoOrderMinusculas = '';

  for (let i = 0; i < s.length; i++) {
    if (s[i] === s[i].toUpperCase()) {
      nuevoOrderMayusculas = nuevoOrderMayusculas + s[i];
    } else {
      nuevoOrderMinusculas = nuevoOrderMinusculas + s[i]
    }
  }
  return nuevoOrderMayusculas + nuevoOrderMinusculas;
}
...
function capicua(numero) {
  ...
  if (numero == numString.split('').reverse().join('')) {
    return 'Es capicua';
  }
